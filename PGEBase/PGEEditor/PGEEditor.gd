...
func _on_graph_node_moved(graph_node) -> void:
	...
	var node_end: Vector2 = graph_node.rect_position + graph_node.rect_size
...
func _on_graph_node_released(node) -> void:
	refresh_panel_size()
...
func add_node_from_graph_recursive(graph: PGEGraph, node_data: Dictionary) -> Node:
	# Get or create node
	...
	if not pge_node:
		...
		for block_data in node_data.blocks:
			...
			var pge_block = pge_node.add_block(load(block_filename))
...
func open_graph(graph: PGEGraph) -> void:
	...
	for node_data in graph.nodes.values():
		add_node_from_graph_recursive(graph, node_data)
